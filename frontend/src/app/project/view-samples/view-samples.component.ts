...
export class ViewSamplesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sub = this.route.params.subscribe(params => {
      ...
      TimerObservable.create(0, 10000)
        ...
        .subscribe(() => {
          this.sampleService
            .getSamplesByProject(this.projectComponent.projectID)
            .subscribe(response => {});
        });
  ...
  generate_gene_arrangement_image(data: any) {
    ...
    data.data.forEach(element => {
      ...
      const _category =
        element._category === 'ARGs' ? element._gene_name : element._class_name;
  ...
  sample_comparison() {
    ...
    samples.forEach((sample, index) => {
      // console.log(sample);
      this.sampleService.get_sample_results(sample['_id']).subscribe(res => {
        // console.log(res);
  ...
  render_network() {
    const _dict_ntw = { ARGs: 1, MGEs: 2, MRGs: 4, Species: 10, Pathogens: 9 };

    const _sel_option_render = this.network_parameters.map(e => {
      return _dict_ntw[e];
    });
