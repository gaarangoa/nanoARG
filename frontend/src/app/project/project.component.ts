...
export class ProjectComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.SUB = this.route.params.subscribe(params => {
      ...
      this.projectService.getProjectById(params['pid']).subscribe(response => {
  ...
  share_project(user_to_share: string) {
    ...
    this.userService.get_user_id_from_username(user_to_share).subscribe(res => {
      this.shared_user_id = res.user_id;

      this.projectService
        .shareProject({
          project_id: this.projectInfo['_id'],
          to_user_id: this.shared_user_id
        })
        .subscribe(res => {
          this.msgs.push({
            severity: 'success',
