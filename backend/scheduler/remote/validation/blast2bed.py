...
def blast2bed(input, arglen, iden, evalue, cov, bitscore):
    ...
    for i in open(input):
        ...
        extra = "_".join([ i[3], i[-2], i[2], str(coverage), str(ARGLen[i[1].split('|')[0]]), i[8], i[9], i[4], i[5] ])

        # read qstart qend reference bitscore strand Alignment-length__Evalue__Identity__Coverage__Reference-gene-length_sstart_send_mismatch_gapopen
