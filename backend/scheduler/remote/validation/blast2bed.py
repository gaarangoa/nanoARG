...
def blast2bed(input, arglen, iden, evalue, cov, bitscore):
    # File with the genes sizes, for computing the coverage.
    ARGLen = {i.split()[0].split("|")[0].replace('>', "")
                      : int(i.split()[1]) for i in open(arglen)}
    fo = open(input+'.bed', 'w')
    ...
    for i in open(input):
        ...
        if float(i[2]) < iden:
            continue  # minimum identity 60
        if float(i[10]) > evalue:
            continue  # minimum evalue 1e-10
        if coverage < cov:
            continue  # minimum coverage 90% ARG
        if float(i[-1]) < bitscore:
            continue  # minimum bitscore to consider
        ...
        wt = "\t".join([i[0], start, end, i[1], i[11], strand, extra])+'\n'
        fo.write(wt)

        fo2.write("\t".join([i[0], start, end, i[1], i[11], strand, str(ARGLen[i[1].split('|')[0]]),
                             str(round(100*float(i[3])/ARGLen[i[1].split('|')[0]], 2)), i[10], i[2]])+'\n')
