...
class DeepARG():
    ...
    def preprocess(self, input_file, arglen_file):
        ...
        for ix, i in enumerate(open(fi+".pre.clusters")):
            ...
            item = [i[0]+"_cluster_"+i[-1], i[3], j[2], j[0],
                    j[7], j[8], start, end, j[5], j[6], j[1], i[4]]
            fo.write("\t".join(item)+"\n")
    ...
    def postprocess(self, input_file):
        print(input_file)
        map_aln = {(i.split()[0], i.split()[1]): i.split()
                   for i in open(input_file+'.tmp')}
        ...
        for i in open(input_file+".dl.ARG"):
            ...
            try:
                j = map_aln[(i[3], i[5])]
                item = "\t".join([i[3].split("_cluster_")[0],
                                  i[5]] + j[2:] + [i[6]])+"\n"
                fo.write(item)
