...
class ARGs():
    def __init__(self, input):
        ...
        self.observable_file = input+"."+self.database_name + \
            ".aln.dl.tmp.bed.clusters.bestHit.annotated."+self.database_name+".json"
        self.postprocess_file = input+"."+self.database_name + \
            ".alg.annotated."+self.database_name+".json"
    ...
    def align(self):
        ...
        self.deeparg.preprocess(self.alignment_file,
                                conf.data+self.database_name+".size")
        self.deeparg.predict(self.alignment_file, self.reference,
                             self.alignment_file+".dl", parameters)
        self.deeparg.postprocess(self.alignment_file)

    def postprocess(self):
        annotation = BestHit(conf.data+self.database_name +
                             ".size", _IDEN, _EVALUE, _COVERAGE, _BITSCORE)
